...
export type RecentActivityItem = {
  id: string
  type: "bug_report" | "solution" | "bug_vote" | "solution_vote"
  title: string
  href: string
  createdAt: string
}
...
async function fetchRecentActivity(userId: string): Promise<RecentActivityItem[]> {
  ...
  const [bugs, solutions, bugVotes, solutionVotes] = await Promise.all([
    supabase
      .from("bugs")
  ...
  const bugIds = [...new Set([...(bugVotes.data ?? []).map((v) => v.bug_id)])]
