...
import { voteOnSolution, getUserSolutionVote } from "@/app/actions/bug/solutionVote"
...
export async function GET(
  ...
  try {
    ...
    if (!solutionId) {
      return errorResponse("Solution ID is required", 400)
    }

    const result = await getUserSolutionVote(ensureValidUUID(solutionId), user.id)
...
export async function POST(
  ...
  try {
    ...
    const result = await voteOnSolution(ensureValidUUID(solutionId), voteType)
